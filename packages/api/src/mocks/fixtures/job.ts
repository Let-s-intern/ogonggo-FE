 ...
 * 그래서 이번 달 격자 안에 확인용 날이 전부 들어온다.
 *
 * | 오늘 기준 | 마감 건수 | 공고 id | 확인하는 것 |
 * |---|---|---|---|
 * | 오늘 | 5 | 30, 60, 90, 120, 188 | 주간 뷰 파랑 막대(PRD 8.3) |
 * | +1일 | 10 | 213, 243, 273, 303, 333, 363, 393, 423, 453, 483 | 로고 6개 + `+4` |
 * | +5일 | 7 | 316, 346, 376, 406, 436, 466, 496 | 7건 경계 — 로고 6개 + `+1` |
 * | +6일 | 6 | 526, 556, 586, 616, 646, 676 | 6건에는 `+N`이 없다 |
 * | +12일 | 0 | 없음 | 빈 날 |
 *
 * id는 오늘이 며칠이든 그대로다 — 아래 `withCalendarDeadlines`가 원래 마감일 순서로 나눠 주고
 * 그 순서 자체가 `daysOut`으로 고정돼 있다.
 */
const CALENDAR_DAY_QUOTAS = [2, 4, 1, 6, 3, 0, 8, 5, 10, 4, 2, 12, 7, 6];
