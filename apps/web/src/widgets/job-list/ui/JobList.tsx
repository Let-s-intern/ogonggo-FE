...
export async function JobList({ page }: JobListProps) {
  const { items } = await fetchJobPage({ page });

  if (items.length === 0) {
    return <p className="py-16 text-center text-sm text-gray-500">채용공고가 없습니다.</p>;
  }
