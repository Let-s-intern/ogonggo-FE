...
import { Button, Select, type SelectOption } from '@ogonggo/ui';
...
export interface MyApplicationRowProps {
  row: Row;
  /**
   * 상태 셀렉트의 선택지. 탭마다 단계 수가 다르고(여섯 / 다섯 / 넷), 옮길 수 없는 단계는
   * 목록에 두되 `disabled` 로 그린다 — 어디까지 있는 흐름인지가 그 자체로 정보다.
   */
...
/**
 * 표 한 줄(목업 `docs/asset/v4 마이페이지/지원 신청내역/`). 앞 두 칸(모집글 정보 · 마감일) 은
 * 스크랩 표와 같은 `MyPageListRowCells` 가 그리고, 뒤 두 칸이 이 화면 것이다.
 *
 ...
 * 여닫기는 `<details>` 라 자바스크립트가 없어도 열린다.
 */
