...
function showsApply(
  ...
  const ids: readonly ApplicationStageId[] = stagesOf(tab).map(
    (stage) => stage.id,
  );
 ...
 */
export function ApplicationBoardRow({
  tab,
  stage,
  item,
  move,
}: ApplicationBoardRowProps) {
    ...
    <div className="flex items-center border-b border-gray-200 px-6 py-5.5">
      {removable ? (
        ...
        >
          <span
            aria-hidden="true"
            className="icon-[lucide--x] block h-3.5 w-3.5"
          />
        </button>
      ) : null}
      <Thumbnail
