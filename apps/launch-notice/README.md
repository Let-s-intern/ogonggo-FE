...
`docs/apps-script.gs` 의 설치 순서를 따른다. **배포 전에 `setup` 함수를 한 번 손으로 실행해야 한다** — 앱스 스크립트는 권한 승인 없이 배포하면 첫 요청에서 조용히 실패한다. 배포 뒤 `/exec` 주소를 브라우저로 열어 `{"ok":true}` 가 보이면 살아 있는 것이다.
