...
/**
 * 백엔드가 아직 없는 네 화면(대시보드·사이드·스터디·일반 회원·비즈니스 회원)이 실서버
 * 모드에서 무엇을 보여줄지 정한다.
 *
 * 목 모드에서는 아홉 메뉴가 지금처럼 목데이터로 그려진다. 실서버 모드에서만 이 넷이 빈
 ...
 * (지면·통계) 에 쓰는 표시고, 이 넷은 화면이 있고 목 모드에서 돈다.
 */
 ...
 * 메뉴마다 다른 안내 문구.
 *
 * 아홉 중 넷이 같은 문장으로 비면 운영자는 콘솔 전체가 고장 난 것으로 읽는다. 무엇이 준비
