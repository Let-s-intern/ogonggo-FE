 ...
 *
 * **검수하는 것은 비즈니스 회원(기업회원)이 올린 채용공고와 부트캠프뿐이다.** 크롤러 수집분은
 * 등록 시점에 게시 상태가 되어 이 큐에 오지 않는다(BE `3467f4a`, LC-3352). 우리가 고른
  ...
  if (queue.length === 0) {
    return (
      <>
        ...
        <EmptyState
          title="검수할 것이 없습니다"
          description="비즈니스 회원이 올린 채용공고와 부트캠프만 검수합니다. 크롤러 수집분은 검수 없이 바로 게시되어 여기 오지 않습니다."
        />
      </>
    );
  }
